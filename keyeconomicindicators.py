...
def refresh_all_data() -> tuple[bool, list[str]]:
    scripts = [
        "download_fedrate.py",
    ...
    for script in scripts:
        try:
            ...
            messages.append(f"OK {script}")
        except Exception as exc:
            ok = False
            messages.append(f"ERROR {script}: {exc}")
...
def build_figure(
    ...
    if show_spread and not plot_ust.empty:
        fig.add_trace(
            go.Scatter(
                ...
                name="10Y-2Y Spread",
                line={"color": "rgba(255,0,0,0.6)", "width": 2, "dash": "dot"},
                yaxis="y2",
            )
        )
    ...
    if show_inflation and not plot_infl.empty:
        fig.add_trace(
            go.Scatter(
                ...
                line={"color": "#C62828", "width": 2.4, "dash": "dot"},
                yaxis="y2",
            )
        )
    ...
    if show_unemployment and not plot_unrate.empty:
        fig.add_trace(
            go.Scatter(
                ...
                line={"color": "rgba(128, 0, 128, 0.6)", "width": 2, "dash": "dot"},
                connectgaps=True,
                yaxis="y2",
            )
        )
    ...
    fig.update_layout(
        xaxis_title="Date",
        yaxis={"title": "Yield (%)", "side": "left", "range": y_range},
        yaxis2={"side": "right", "overlaying": "y", "showgrid": False, "range": y_range},
        hovermode="x unified",
    ...
    [
        ...
        html.Div(
            [
                ...
                html.Div("Series", className="control-label"),
                dcc.Checklist(
                    id="show-yields",
                    options=[{"label": "Show Treasury Yields", "value": "on"}],
                    value=["on"],
                ...
                dcc.Checklist(
                    id="show-spread",
                    options=[{"label": "Show 10Y-2Y Spread", "value": "on"}],
                    value=[],
                    className="control-group",
                ),
                dcc.Checklist(
                    id="show-fed-rate",
                    options=[{"label": "Show Federal Reserve Rate", "value": "on"}],
                    value=["on"],
                    className="control-group",
                ),
                dcc.Checklist(
                    id="show-inflation",
                    options=[{"label": "Show Core PCE Inflation", "value": "on"}],
                    value=["on"],
                    className="control-group",
                ),
                dcc.Checklist(
                    id="show-unemployment",
                    options=[{"label": "Show Unemployment Rate", "value": "on"}],
                    value=[],
                    className="control-group",
                ),
                dcc.Checklist(
                    id="show-unemp-ind",
                    options=[{"label": "Show Unemployment Indicator (U3-NROU)", "value": "on"}],
                    value=[],
                    className="control-group",
                ),
...
def refresh_data(n_clicks: int, token: int):
    if not n_clicks:
        return no_update, no_update

    ok, messages = refresh_all_data()
    color = "#1b5e20" if ok else "#b00020"
    return html.Div([html.Div(m) for m in messages], style={"color": color}), (token or 0) + 1
...
def update_visuals(
    ...
    if (show_unemployment or show_unemp_ind) and not unrate_monthly.empty:
    ...
    return fig, f"Treasury Data: {freq_label}", warning
